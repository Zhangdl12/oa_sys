...
from services.oa_admin.apps.external.models.feishu import FeishuNotifyRequest
...
async def send_feishu_notify(
    ...
    用途：
        校验发送权限后调用飞书业务层，并把当前用户和 request_id 传给发送记录。
    返回值：
        统一响应结构，data 为飞书消息发送结果。
    """
