...
def get_permission_management(
    ...
    用途：
        从 FastAPI 依赖中注入 MySQL 和 Redis，并创建 PermissionManagement。
    参数：
        mysql_pool：MySQL 异步连接池。
        redis_client：Redis 异步客户端。
    返回值：
        PermissionManagement 实例。
    """

    return PermissionManagement(mysql_pool=mysql_pool, redis_client=redis_client)


def get_permission_notification_management(
    ...
    用途：
        从 FastAPI 依赖中注入配置和统一通知业务对象，并创建 PermissionNotificationManagement。
    参数：
        settings：系统配置对象。
        notification_management：统一通知业务对象，由 external deps 层组装。
    返回值：
        PermissionNotificationManagement 实例。
    """

    return PermissionNotificationManagement(
        settings=settings,
